...
function saveSubscription(subscription) {
    ...
    xmlHttp.onreadystatechange = function () {
        ...
        if (xmlHttp.status != 200 && xmlHttp.status != 304) {
            ...
            console.log("User subscribed to server"+ xmlHttp.status);
        }
    };
    let subscription_temp = {
        "phonenumber" : "8939668158",
        "subscriptionKey": subscription
    }
    //subscription["phonenumber"] = "8939668158"
    console.log("subscription--", JSON.stringify(subscription_temp))
    xmlHttp.send(JSON.stringify(subscription_temp));
}